...
3. Selector matching rules — how the positional selector matches the message's "control" widget and how keyword selectors match additional message-exposed widgets.
...
- "Control" attribute on a message class as the default selector target.
...
- Supplying a selector for a message type that does not expose a control widget is a registration-time error.
...
- A handler registered for a button-press message filtered by an `id` selector.
